import React, { memo, useState, useEffect, useCallback, useRef, ElementRef } from 'react'
...
const RadioCategoryHeader: FC<IProps> = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    const array = Array(page).fill(0).map((_, i) => i)
    setPages(array)
  }, [categories])
  ...
  return (
    <RadioCategoryHeaderWrapper>
      <button className='sprite_radio_slider arrow left' 
        disabled={currentPage === 0} 
        onClick={e => pageRef.current?.prev()} 
      />
      <Carousel className='category-list' 
        dots={{className: 'dots'}} 
        ref={pageRef} 
          ...
          pages.map(page => {
            return (
              <div className='category-page' key={page}>
                {
                  categories.slice(page * pageSize, (page + 1) * pageSize).map((item, idx) => {
                    return (
                      <NavLink key={item.id}
                        to={`/discover/djradio/category?id=${item.id}`} 
                        className={ classNames('category-item sprite_radio', {selected: id === item.id})}
                        >
                        <RadioCategoryImage className='icon' bgImage={item.picWebUrl} />
                        <span>{item.name}</span>
                      </NavLink>
                    )
                  })
      ...
      <button className='sprite_radio_slider arrow right' 
